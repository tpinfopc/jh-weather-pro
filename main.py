"""
JH WEATHER PRO v3.1 — WEB EDITION
By Juan Heit — Estilo PC Analyzer Pro / Suite JH LAB

Cambios v3.1:
  1. Ajuste de umbrales de alerta para que coincidan con los indicadores del pronóstico.
  2. Alerta amarilla: >= 15 mm de lluvia en 24h.
  3. Alerta naranja: >= 40 mm.
  4. Alerta roja: >= 80 mm (o >=150 mm para extrema).
  5. Scroll horizontal en contenedor de alertas para móviles.
  6. Consistencia entre íconos del pronóstico y alertas reales.
"""
...
# NUEVOS UMBRALES (alineados con los íconos 🟡🟠🔴 del pronóstico)
THRESHOLDS = {
    "rain_yellow": 15.0,      # mm en 24h - ALERTA AMARILLA (coincide con 🟡)
    "rain_orange": 40.0,      # mm en 24h - ALERTA NARANJA (coincide con 🟠)
    "rain_red":    80.0,      # mm en 24h - ALERTA ROJA   (coincide con 🔴)
    "rain_extreme":150.0,     # mm en 24h - ALERTA ROJA EXTREMA
...
TEXTS = {
    "es": {
        ...
        "alert_yellow_desc":    "Lluvias moderadas. Permanece atento a posibles acumulados.",
        "alert_orange_desc":    "Lluvias intensas. Riesgo de anegamientos. Evita actividades al aire libre.",
        "alert_red_desc":       "Lluvias extremas o tormentas severas. Peligro para la población.",
        "alert_storm_warning":  "Tormentas fuertes con ráfagas de viento > 70 km/h",
        "alert_heavy_rain":     "Lluvias intensas: {}mm en 24h",
        "alert_extreme_rain":   "LLUVIA EXTREMA: {}mm en 24h",
        "alert_extreme_wind":   "Vientos extremos > 90 km/h",
        "alert_hail":           "Posibilidad de granizo",
    ...
    "en": {
        ...
        "alert_red":            "RED ALERT",
        "alert_yellow_desc":    "Moderate rain. Stay aware of possible accumulations.",
        "alert_orange_desc":    "Heavy rain. Risk of flooding. Avoid outdoor activities.",
        "alert_red_desc":       "Extreme rain or severe storms. Danger to population.",
        "alert_storm_warning":  "Severe storms with wind gusts > 70 km/h",
        "alert_heavy_rain":     "Heavy rain: {}mm in 24h",
        "alert_extreme_rain":   "EXTREME RAIN: {}mm in 24h",
        "alert_extreme_wind":   "Extreme winds > 90 km/h",
        "alert_hail":           "Possibility of hail",
...
class WeatherPro:
    def __init__(self, page: ft.Page):
        ...
        self.current_weather_container = ft.Container(
            content=ft.Column(controls=[ft.Text("⏳ Detectando...", color=C["muted"], size=12)], spacing=10, horizontal_alignment=ft.CrossAxisAlignment.CENTER),
            bgcolor=C["panel"], border_radius=10, padding=20,
        )
        self.forecast_container = ft.Container(
            content=ft.Column(controls=[ft.Text("⏳ Detectando...", color=C["muted"], size=12)], spacing=10),
            bgcolor=C["panel"], border_radius=10, padding=20,
        )
        ...
        self.alerts_scroll = ft.Row(spacing=12, scroll=ft.ScrollMode.AUTO)
        ...
        self.alert_banner = ft.Container(
            ...
            content=ft.Row(controls=[ft.Text("⚠️", size=20, color=C["alert_text"]), ft.Text("", color=C["alert_text"], size=12, weight=ft.FontWeight.BOLD)], spacing=10),
        )
    ...
    def _fetch_one_call_alerts(self, lat: float, lon: float):
        try:
            params = {"lat": lat, "lon": lon, "appid": API_KEY, "exclude": "minutely,hourly,daily", "units": "metric"}
    ...
    def _build_ui(self):
        ...
        header = ft.Container(
            content=ft.Row(
                controls=[
                    ...
                    ft.Container(content=ft.Row(controls=[ft.Text("🔧", size=14, color=C["green"]), self.status_text], spacing=5)),
                    ft.Row(controls=[
                        ft.Text(self.txt["unit_celsius"], size=11, color=C["label"]), self.unit_switch,
                        ft.Text(self.txt["lang_es"], size=10, color=C["accent"] if self.language == "es" else C["muted"]),
                        self.lang_switch,
                        ft.Text(self.txt["lang_en"], size=10, color=C["accent2"] if self.language == "en" else C["muted"]),
                        self.clock_text,
                    ], alignment=ft.MainAxisAlignment.END, spacing=5, wrap=True),
                ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN, wrap=True,
            ), bgcolor="#070a0d", padding=ft.padding.symmetric(horizontal=20, vertical=10),
        )
        ...
        alert_banner_container = ft.Container(content=self.alert_banner, padding=ft.padding.symmetric(horizontal=20, vertical=5))
        ...
        ], spacing=15, alignment=ft.MainAxisAlignment.START, wrap=True, vertical_alignment=ft.CrossAxisAlignment.START)
        middle_row = ft.Row(controls=[
            ft.Container(content=self.wind_vis_container, width=240),
            ft.Container(content=self.forecast_container, width=900),
        ], spacing=15, alignment=ft.MainAxisAlignment.START, wrap=True, vertical_alignment=ft.CrossAxisAlignment.START)
        location_block = ft.Container(content=ft.Column(controls=[ft.Text(self.txt["label_location"], size=11, color=C["label"]), self.city_display], spacing=5), padding=ft.padding.only(left=20, top=20))
        footer = ft.Container(content=ft.Column(controls=[ft.Divider(color=C["border"]), ft.Text(self.txt["footer_ref"], size=9, color=C["muted"]), ft.Text(self.txt["footer_features"], size=8, color=C["muted"])], spacing=5, horizontal_alignment=ft.CrossAxisAlignment.CENTER), padding=ft.padding.all(10))
        tip = ft.Container(content=ft.Text(self.txt["tip_click"], size=9, color=C["muted"], italic=True), padding=10)
        self.page.add(header, button_bar, alert_banner_container, location_block, ft.Container(content=top_row, padding=ft.padding.symmetric(horizontal=20)), ft.Container(content=middle_row, padding=ft.padding.symmetric(horizontal=20)), self.alerts_container, tip, footer)
    ...
    def _create_button(self, text, on_click, bg, fg, border_color=None):
        return ft.ElevatedButton(text=text, on_click=on_click, style=ft.ButtonStyle(bgcolor=bg, color=fg, overlay_color=C["border"], shape=ft.RoundedRectangleBorder(radius=5), side=ft.BorderSide(1, border_color) if border_color else None))
    ...
    def _process_hourly_data_by_day(self):
        ...
        for date_key in self.daily_forecasts_data:
            ...
            self.daily_forecasts_data[date_key]["hourly"] = self._interpolate_temperatures(self.daily_forecasts_data[date_key]["hourly"])
        ...
        for date_key, day_data in self.daily_forecasts_data.items():
            ...
            for hd in hourly:
                ...
                if 200 <= wid <= 232:
                    ...
                    max_wid = wid
            # Descripción con nuevos umbrales
            if total_rain >= THRESHOLDS["rain_red"]:
                description = f"Lluvia extrema: {total_rain:.0f}mm"
            elif total_rain >= THRESHOLDS["rain_orange"]:
                description = f"Lluvia intensa: {total_rain:.0f}mm"
            elif total_rain >= THRESHOLDS["rain_yellow"]:
                description = f"Lluvia significativa: {total_rain:.0f}mm"
            elif total_rain > 0:
            ...
            self.daily_summary[date_key] = {"total_rain": total_rain, "description": description, "weather_id": max_wid, "period": period_w_rain}
    ...
    def _analyze_heuristic_alerts(self):
        if not self.forecast_data or not self.current_weather:
            return
        total_rain_24h = 0
        max_wind_24h = self.current_weather.get("wind", {}).get("speed", 0) * 3.6
        has_storm = False
        max_hail_risk = False
        for item in self.forecast_data.get("list", []):
            if "rain" in item:
                total_rain_24h += item["rain"].get("3h", item["rain"].get("1h", 0) * 3)
            weather_id = item["weather"][0]["id"]
        ...
        alert_level = None
        alert_reasons = []
        # Evaluar lluvia con nuevos umbrales
        if total_rain_24h >= THRESHOLDS["rain_red"]:
            alert_level = "red"
            alert_reasons.append(self.txt["alert_extreme_rain"].format(int(total_rain_24h)))
            alert_reasons.append(f"Acumulado: {total_rain_24h:.1f}mm en 24h")
        elif total_rain_24h >= THRESHOLDS["rain_orange"]:
            alert_level = "orange"
            alert_reasons.append(self.txt["alert_heavy_rain"].format(int(total_rain_24h)))
            alert_reasons.append(f"Acumulado: {total_rain_24h:.1f}mm en 24h")
        elif total_rain_24h >= THRESHOLDS["rain_yellow"]:
            alert_level = "yellow"
            alert_reasons.append(f"Lluvias significativas: {total_rain_24h:.1f}mm en 24h")

        # Tormentas (sube nivel si es necesario)
        if has_storm:
        ...
        if alert_level and alert_reasons:
            titles = {"yellow": (self.txt["alert_yellow"], "🟡"), "orange": (self.txt["alert_orange"], "🟠"), "red": (self.txt["alert_red"], "🔴")}
    ...
    def _update_alerts_display(self):
            ...
            no_alerts_card = ft.Container(
                content=ft.Row(controls=[
                    ...
                    ft.Column(controls=[
                        ...
                        ft.Text("Las condiciones meteorológicas actuales no presentan riesgos significativos.", size=11, color=C["muted"])
                    ], spacing=4)
                ], spacing=12, vertical_alignment=ft.CrossAxisAlignment.CENTER),
                bgcolor=C["bg"], border_radius=10, padding=12, border=ft.border.all(1, C["border"]), width=900
            )
        ...
        for alert in self.active_alerts:
            ...
            if alert.details:
                ...
                if details_text:
                    alert_controls.append(ft.Text(details_text, size=10, color=tx_c, wrap=True))
            if alert.is_official and alert.sender:
                alert_controls.append(ft.Text(self.txt["from_agency"].format(alert.sender), size=9, color=tx_c, italic=True))
            alert_card = ft.Container(content=ft.Column(controls=alert_controls, spacing=8), bgcolor=bg_c, border_radius=10, padding=12, width=320)
    ...
    def _update_current_weather(self):
        if not self.current_weather:
            self.current_weather_container.content = ft.Column(controls=[ft.Text("⏳ Cargando datos...", size=14, color=C["muted"])], spacing=10, horizontal_alignment=ft.CrossAxisAlignment.CENTER)
            self.page.update()
            return
        ...
        self.current_weather_container.content = ft.Column(controls=[
            ft.Row(controls=[ft.Text(icon, size=48), ft.Column(controls=[ft.Text(f"{ctmp:.1f}{self._get_temp_unit()}", size=36, weight=ft.FontWeight.BOLD, color=tc), ft.Text(wdesc.capitalize(), size=14, color=C["label"])], spacing=5, horizontal_alignment=ft.CrossAxisAlignment.CENTER)], alignment=ft.MainAxisAlignment.CENTER, spacing=20),
            cards
        ], spacing=10, horizontal_alignment=ft.CrossAxisAlignment.CENTER)
        self.page.update()

    def _create_info_card(self, icon, label, value, color):
        return ft.Container(content=ft.Column(controls=[ft.Text(icon, size=24), ft.Text(label, size=10, color=C["label"]), ft.Text(value, size=14, weight=ft.FontWeight.BOLD, color=color)], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=5), bgcolor=C["bg"], border_radius=8, padding=10, width=120, border=ft.border.all(1, C["border"]))

    def _update_forecast(self):
        if not self.forecast_data:
            self.forecast_container.content = ft.Column(controls=[ft.Text("⏳ Cargando pronóstico...", size=12, color=C["muted"])], spacing=11)
            self.page.update()
            return
        ...
        for item in forecast_list:
            ...
            if date_key not in daily_forecasts and len(daily_forecasts) < 5:
                daily_forecasts[date_key] = {"date": dt, "temp": item["main"]["temp"], "weather_id": item["weather"][0]["id"], "weather_desc": item["weather"][0]["description"], "day_name": SHORT_DAY_NAMES[self.language][weekday]}
        forecast_cards = []
        for date_key, data in daily_forecasts.items():
            ...
            card = ft.GestureDetector(on_tap=lambda e, dk=date_key: self._on_day_click(dk), content=ft.Container(content=ft.Column(controls=[ft.Text(data["day_name"], size=14, weight=ft.FontWeight.BOLD, color=C["accent"]), ft.Text(data["date"].strftime("%d/%m"), size=10, color=C["label"]), ft.Text(icon, size=32), ft.Text(f"{ctmp:.0f}{self._get_temp_unit()}", size=18, weight=ft.FontWeight.BOLD, color=tc), ft.Text(desc_text, size=10, color=C["muted"]), ft.Text(rain_text, size=10, color=C["rain_color"] if total_rain > 0 else C["muted"])], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=5), bgcolor=C["bg"], border_radius=10, padding=12, width=120, border=ft.border.all(1, C["border"])))
            forecast_cards.append(card)
        self.forecast_container.content = ft.Row(controls=forecast_cards, spacing=15, wrap=True, alignment=ft.MainAxisAlignment.CENTER)
        self.page.update()

    def _update_wind_visibility(self):
        if not self.current_weather:
            self.wind_vis_container.content = ft.Column(controls=[ft.Text("⏳ Cargando...", size=12, color=C["muted"])], spacing=11)
            self.page.update()
            return
        ...
        wkmh = wsp * 3.6
        self.wind_vis_container.content = ft.Column(controls=[
            ft.Text(self.txt["label_wind_vis"], size=10, color=C["accent2"], weight=ft.FontWeight.BOLD),
            ft.Divider(color=C["border"]),
            ft.Row(controls=[ft.Text("🌬️", size=24), ft.Column(controls=[ft.Text(f"{wkmh:.1f} km/h", size=16, weight=ft.FontWeight.BOLD, color=C["text"]), ft.Text(f"{self.txt['label_direction']}: {wdir}", size=11, color=C["label"])], spacing=2)], spacing=10),
            ft.Row(controls=[ft.Text("👁️", size=24), ft.Column(controls=[ft.Text(f"{vis:.1f} km", size=16, weight=ft.FontWeight.BOLD, color=C["text"]), ft.Text(self.txt["label_visibility"], size=11, color=C["label"])], spacing=2)], spacing=10),
        ], spacing=10)
        self.page.update()
    ...
    def _update_trend_display_for_day(self, date_key: str):
        ...
        bars_row = ft.Row(controls=[], spacing=8, alignment=ft.MainAxisAlignment.CENTER, scroll=ft.ScrollMode.AUTO)
        for hd in hourly:
            tc = hd["temp"]
            ...
            bars_row.controls.append(ft.Column(controls=[ft.Text(icon, size=22), ft.Container(width=40, height=h, bgcolor=col, border_radius=ft.border_radius.only(top_left=5, top_right=5), opacity=op), ft.Text(f"{td:.0f}{self._get_temp_unit()}", size=12, color=C["text"]), ft.Text(hd["hour"], size=12, color=C["muted"]), ft.Text(ri, size=10, color=C["rain_color"] if pop > 0 else C["muted"])], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=4))
        self.trend_container.content = ft.Column(controls=[ft.Row(controls=[ft.Text(f"📅 {day_name} {date_str}", size=12, color=C["accent"], weight=ft.FontWeight.BOLD), ft.Text(" • ", size=10, color=C["muted"]), ft.Text(f"{self.txt['label_min']}: {self._convert_temp(min_temp):.1f}{self._get_temp_unit()}", size=10, color=C["temp_cold"]), ft.Text(f"{self.txt['label_max']}: {self._convert_temp(max_temp):.1f}{self._get_temp_unit()}", size=10, color=C["temp_hot"]), ft.Text(f"• {len(hourly)} {self.txt['label_measurements']}", size=9, color=C["muted"])], spacing=5), ft.Divider(color=C["border"]), bars_row], spacing=10)
        self.page.update()
    ...
    def _update_trend_display(self):
        ...
        for hd in self.hourly_forecast:
            ...
            h = 30 + ((tc - min_temp) / (max_temp - min_temp) * 70) if max_temp > min_temp else 65
            bars_row.controls.append(ft.Column(controls=[ft.Text(icon, size=20), ft.Container(width=40, height=h, bgcolor=col, border_radius=ft.border_radius.only(top_left=5, top_right=5)), ft.Text(f"{td:.0f}{self._get_temp_unit()}", size=9, color=C["text"]), ft.Text(hd["hour"], size=9, color=C["muted"]), ft.Text(ri, size=8, color=C["rain_color"] if pop > 0 else C["muted"])], horizontal_alignment=ft.CrossAxisAlignment.CENTER, spacing=4))
        self.trend_container.content = ft.Column(controls=[ft.Text(f"{self.txt['label_trend']} (24h)", size=10, color=C["accent"], weight=ft.FontWeight.BOLD), ft.Text(f"{self.txt['label_min']}: {self._convert_temp(min_temp):.1f}{self._get_temp_unit()}  |  {self.txt['label_max']}: {self._convert_temp(max_temp):.1f}{self._get_temp_unit()}", size=9, color=C["muted"]), bars_row], spacing=10)
        self.page.update()
    ...
    def share_weather(self, e):
        ...
        share_text = f"🌤️ JH WEATHER PRO - Reporte Meteorológico\n📍 {self.city_name}\n🌡️ Temperatura: {self._convert_temp(temp):.1f}{self._get_temp_unit()}\n☁️ Condición: {desc.capitalize()}\n💧 Humedad: {hum}%{alert_text}\n📅 {datetime.datetime.now().strftime('%d/%m/%Y %H:%M')}\n\nDatos proporcionados por OpenWeatherMap"
    ...
    def _show_share_confirmation(self):
        def close(e):
            dialog.open = False
            self.page.update()
        dialog = ft.AlertDialog(title=ft.Text(self.txt["dialog_title_copied"], color=C["green"]), content=ft.Text(self.txt["dialog_text_copied"], color=C["text"]), actions=[ft.TextButton("Aceptar" if self.language == "es" else "OK", on_click=close)])
    ...
    def _show_api_key_message(self):
        def close(e):
            dialog.open = False
            self.page.update()
        dialog = ft.AlertDialog(title=ft.Text(self.txt["dialog_title_apikey"], color=C["warn"], weight=ft.FontWeight.BOLD), content=ft.Column(controls=[ft.Text(self.txt["dialog_text_apikey"], color=C["text"]), ft.Text(self.txt["dialog_text_pending"], color=C["warn"], weight=ft.FontWeight.BOLD), ft.Text(self.txt["dialog_suggestions"], color=C["accent"]), ft.Text(self.txt["dialog_suggestion1"], color=C["muted"]), ft.Text(self.txt["dialog_suggestion2"], color=C["muted"])], spacing=10), actions=[ft.TextButton("Cerrar" if self.language == "es" else "Close", on_click=close)])
        self.page.dialog = dialog
        dialog.open = True
        self.page.update()
